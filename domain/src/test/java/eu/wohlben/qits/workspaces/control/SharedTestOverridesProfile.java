 ...
 *
 * <p>It is deliberately not a theme. The three overrides below have nothing to do with each other —
 * a chain-await bound and a git identity — and that is precisely the point: a {@code
 * @TestProfile} is a QUARKUS RESTART, Quarkus keys applications by profile IDENTITY rather than by
 * the map they produce, and two classes each carrying a map of its own were two restarts whose
 ...
 * Union what does not conflict and you pay for one application instead of two.
 *
 ...
 *
 * <p>It held a fourth override until the editor became one container for the platform: {@code
 * qits.editor.label-miss-ttl-ms}, the window {@code EditorProxyTargetsTest} shortened so waiting one
 * out was a second rather than the shipped five. That cache is gone with the per-project lookup and
 * the key is retired, so the class that needed the profile does not need one at all any more.
 *
 * <p>Sharing an application means sharing the {@code @Singleton} fakes across both classes'
